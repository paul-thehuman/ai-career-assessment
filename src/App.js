...
// Helper function to render Markdown to HTML (used for in-app display)
...
const App = () => {
  ...
  // FIX: Removed showReportModal and setshowReportModal as they are no longer used
  // const [showReportModal, setShowReportModal] = useState(false);
  // FIX: Removed reportDownloadContent and setReportDownloadContent as they are no longer used
  ...
  // FIX: Wrapped initialCoreQuestions in useMemo to prevent re-creation on every render
  ...
  useEffect(() => {
    if (aiReport || skillGapAnalysis) {
      // No longer setting reportDownloadContent as a downloadable file, but keeping the function for internal use if needed
      // setReportDownloadContent(generateHtmlReport({ aiReport, skillGapAnalysis }, userProfile, colors));
    }
  }, [aiReport, skillGapAnalysis, userProfile]); // Removed reportDownloadContent from dependencies as it's not directly used for display

  // Removed handleDownloadReport as it's no longer needed
            ...
            <div className="p-8 rounded-xl shadow-2xl max-w-3xl w-full" style={{ backgroundColor: colors.lightGrey, color: colors.deepBlack }}>
              ...
              {isLoading ? (
                ...
                <>
                  ...
                  {/* Skill Gap Analysis will be rendered by MarkdownRenderer if present */}
                </>
              )}
            </div>
          </div>
